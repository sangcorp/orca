...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    allowedFlags: [...GLOBAL_FLAGS, 'worktree', 'command', 'title', 'agent', 'focus'],
    notes: [
      'Creates a visible terminal tab without switching focus when possible; falls back to a background handle if the UI cannot adopt it. Pass --focus to switch to it.',
      'Pass --agent to stamp launchAgent on the tab so it appears in the Agents list even when the foreground process is not a known TUI (e.g. a Multica bridge).',
    ...
    examples: [
      ...
      'orca terminal create --worktree path:/projects/myapp --title "[ceo-assistant] task" --command "multica-bridge" --agent claude1'
    ]
  },
  {
