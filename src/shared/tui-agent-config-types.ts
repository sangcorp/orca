// The TuiAgentConfig shape and its supporting unions, split out of
// tui-agent-config.ts so that file stays inside the max-lines budget as the
// agent table grows. Re-exported from tui-agent-config.ts, so existing
