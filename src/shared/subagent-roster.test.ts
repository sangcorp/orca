...
describe('Codex subagent roster', () => {
  it('normalizes retained identity fields before storing them', () => {
    ...
    const snapshot = rosterToSubagentSnapshots(roster)?.[0]
  ...
  it('rejects an id that would normalize to an invisible child', () => {
    ...
    upsertSubagent(roster, '   ', { state: 'waiting' }, 10)
  ...
  it('bounds live storage while admitting a replacement after one child stops', () => {
    ...
    for (let index = 0; index <= AGENT_STATUS_MAX_SUBAGENTS; index += 1) {
      upsertSubagent(roster, `child-${index}`, { state: 'working' }, index)
    }

    expect(roster.size).toBe(AGENT_STATUS_MAX_SUBAGENTS)
    expect(roster.has(`child-${AGENT_STATUS_MAX_SUBAGENTS}`)).toBe(false)

    finishSubagent(roster, 'child-0')
    upsertSubagent(roster, 'replacement', { state: 'working' }, 100)
  ...
  describe('setSubagentModel', () => {
    it('records the model without disturbing the child lifecycle or label', () => {
      const roster: SubagentRoster = new Map()
      upsertSubagent(roster, 'child-1', { description: '/root/audit', state: 'waiting' }, 10)

      setSubagentModel(roster, 'child-1', ' gpt-5.6-terra ')
    ...
    it('never creates a row for a child that is no longer tracked', () => {
      const roster: SubagentRoster = new Map()
      upsertSubagent(roster, 'child-1', { state: 'working' }, 10)
      finishSubagent(roster, 'child-1')
    ...
    it('keeps a known model when the new value is empty', () => {
      const roster: SubagentRoster = new Map()
      upsertSubagent(roster, 'child-1', { model: 'gpt-5.6-sol', state: 'working' }, 10)

      setSubagentModel(roster, 'child-1', '   ')
      setSubagentModel(roster, 'child-1', undefined)
    ...
    it('bounds an oversized model to the shared cap', () => {
      ...
      setSubagentModel(roster, 'child-1', 'x'.repeat(AGENT_MODEL_MAX_LENGTH + 50))
