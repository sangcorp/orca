...
export function getOpenCodeFamilyPluginSource(
  ...
  return [
    ...
    '}',
    '',
    '// The one OpenCode tool that spawns a child agent. Kept in sync with the',
    '// spawn-tool table in shared/subagent-spawn-tools.ts, which reads the',
    '// SubagentStart payload back into a roster row.',
    'const SUBAGENT_SPAWN_TOOL = "task";',
    '',
    ...
    '  },',
    '',
    '  // Why: OpenCode has no subagent lifecycle event of its own — spawning a',
    "  // child IS a call to the `task` tool. Reporting that one tool's",
    '  // boundaries gives Orca the child rows it draws for Claude and Codex.',
    '  // Scoped to the spawn tool deliberately: the status stream above is',
    '  // throttled on purpose, and posting for every tool would flood it.',
    '  // Both handlers swallow their own failures — a status hook must never',
    '  // fail a real tool call.',
    '  "tool.execute.before": async (input, output) => {',
    '    if (disposed || !input || input.tool !== SUBAGENT_SPAWN_TOOL) return;',
    '    try {',
